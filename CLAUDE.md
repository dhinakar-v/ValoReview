...
**No positions exist.** The replication stream's property payloads are undecoded, so there is no player position, rotation or map geometry anywhere in this pipeline. The scene is an explicit schematic and captions itself as one — do not let anything imply a minimap. (`assets/manifest.json` from `fetch-assets` carries Riot's world→radar transforms for the day positions do land; nothing consumes them yet.)
