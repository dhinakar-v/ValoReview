...
def internal_load_object(reader, cache: GuidCache, is_exporting: bool,
                         depth: int = 0) -> int:
    ...
    if depth > MAX_OUTER_RECURSION:
        raise NetError(f"outer chain deeper than {MAX_OUTER_RECURSION}")
    ...
    if guid == 1 or is_exporting:
        ...
        if flags & EXPORT_HAS_PATH:
            outer = internal_load_object(reader, cache, is_exporting, depth + 1)
            path = reader.read_fstring()
            checksum = None
            if flags & EXPORT_HAS_NETWORK_CHECKSUM:
                checksum = reader.read_u32()
            cache.record(
                NetGuidEntry(guid=guid, path=path, outer_guid=outer,
                             checksum=checksum, flags=flags)
            )
...
def read_net_export_guids(reader, cache: GuidCache) -> int:
    ...
    for _ in range(count):
        ...
        if length < 0 or length > reader.bits_left // 8:
            raise NetError(f"bad GUID blob length {length}")
...
def _load_guid_blob(blob: bytes, cache: GuidCache) -> None:
    ...
    if sub.pos != len(blob):
        raise NetError(
            f"GUID blob consumed {sub.pos} of {len(blob)} bytes"
        )
...
class ExportTable:
    ...
    def declare(self, index: int, path: str, num_exports: int
                ) -> NetFieldExportGroup:
...
def read_net_field_export(reader, group: NetFieldExportGroup | None,
...
def read_net_field_exports(reader, table: ExportTable) -> tuple[int, list[str]]:
    ...
    for _ in range(count):
        ...
        if reader.read_int_packed():
            ...
            if group is None:
                raise NetError(f"export references unknown PathNameIndex {index}")
        read_net_field_export(reader, group)
    return count, new_paths


def read_export_data(reader, cache: GuidCache, table: ExportTable
