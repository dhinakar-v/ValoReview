...
def read_packet_bunches(data: bytes, features: Features,
                        keep_payload: bool = True) -> list[Bunch]:
...
def score_features(packets, features: Features,
                   limit: int | None = None) -> PacketScore:
    ...
    for i, packet in enumerate(packets):
        ...
        score.total += 1
        try:
            bunches = read_packet_bunches(packet.data, features,
                                          keep_payload=False)
