...
from vrfnet.calibrate import calibrate, load, save
    ...
    for path in paths:
        if path.lower().endswith(".vrf"):
            ...
            for i, block in enumerate(vrf.data_blocks(kinds=(REPLAYDATA,))):
                raw = oodle.decompress(
                    block.blob(vrf.data), block.decompressed_size
                )
                yield f"block{i:03d}", raw
        else:
            with open(path, "rb") as fh:
                yield path.rsplit("/", 1)[-1], fh.read()


def _session(paths: list[str], oodle_dll: str | None,
             decode_bunches: bool = True) -> ReplaySession:
...
def cmd_calibrate(args) -> int:
    ...
    packets = []
    for label, raw in _blocks_from_args(args.paths, args.oodle_dll):
...
def cmd_actors(args) -> int:
    ...
    print(f"{len(session.channels):,} open channels, "
          f"{len(table):,} distinct archetypes\n")
    for path, channels in sorted(table.items(), key=lambda kv: -len(kv[1])):
        idxs = ", ".join(str(c.ch_index) for c in sorted(
            channels, key=lambda c: c.ch_index)[:12])
...
def main(argv=None) -> int:
    ...
    parser.add_argument("command",
                        choices=("calibrate", "decode", "actors", "exports",
                                 "replay"))
    ...
    parser.add_argument("--out", default="vrfnet/calibrated.json")
