...
class SceneView:
    ...
    def __init__(self, master: tk.Misc, replay: Replay) -> None:
        ...
        self.canvas = tk.Canvas(
            master, bg=theme.BACKGROUND, highlightthickness=0, bd=0
        )
        self.layers = {
            "kills": True,
            "trails": False,
    ...
    def _build(self) -> None:
        ...
        for p in self.replay.players:
            ...
            self._nodes[p.actor_id] = {
                ...
                "dead": c.create_text(0, 0, text="✕", fill=theme.MUTED,
                                      font=FONT_DEAD, state="hidden"),
                "ult": c.create_oval(0, 0, 0, 0, fill=theme.ULT, outline="",
                                     state="hidden"),
                "label": c.create_text(0, 0, text=p.label, fill=theme.BACKGROUND,
                                       font=FONT_LABEL),
                "sub": c.create_text(0, 0, text=f"#{p.actor_id}", fill=theme.MUTED,
                                     font=FONT_SUB),
                "kd": c.create_text(0, 0, text="", fill=theme.MUTED, font=FONT_KD),
            }
        self._arrows = [
            c.create_line(0, 0, 0, 0, fill=theme.MUTED, width=3, smooth=True,
                          arrow=tk.LAST, arrowshape=(11, 13, 4), state="hidden")
        ...
        self._chrome = {
            "a_head": c.create_text(0, 0, text="", fill=theme.team_colour(TEAM_A),
                                    font=FONT_HEAD),
            "b_head": c.create_text(0, 0, text="", fill=theme.team_colour(TEAM_B),
                                    font=FONT_HEAD),
            "a_count": c.create_text(0, 0, text="", fill=theme.team_colour(TEAM_A),
                                     font=FONT_COUNT),
            "b_count": c.create_text(0, 0, text="", fill=theme.team_colour(TEAM_B),
                                     font=FONT_COUNT),
            "banner": c.create_text(0, 0, text="", fill=theme.TEXT,
                                    font=FONT_BANNER),
            ...
            "spike": c.create_polygon(0, 0, 0, 0, 0, 0, 0, 0, fill="",
                                      outline=theme.FAINT, width=2),
            "spike_text": c.create_text(0, 0, text="", fill=theme.MUTED,
                                        font=FONT_SMALL),
            "mark": c.create_text(0, 0, text="SCHEMATIC - node positions are layout, "
                                            "not map coordinates",
                                  fill=theme.FAINT, font=FONT_SMALL, anchor="se"),
        }
    ...
    def _render_nodes(self, snap: Snapshot) -> None:
        ...
        for p in self.replay.players:
            ...
            if alive:
                ...
                c.itemconfigure(
                    item["oval"], fill=theme.BACKGROUND, outline=faded, dash=(3, 3)
                )
    ...
    def _render_chrome(self, snap: Snapshot) -> None:
        ...
        c.itemconfigure(
            self._chrome["swap"], text="sides swapped *" if swapped else ""
        )

        alive_a = sum(
            1 for p in self.replay.team(TEAM_A) if snap.is_alive(p.actor_id)
        )
        alive_b = sum(
            1 for p in self.replay.team(TEAM_B) if snap.is_alive(p.actor_id)
        )
    ...
    def _render_spike(self, snap: Snapshot) -> None:
        ...
        if snap.spike_state == SPIKE_PLANTED:
            ...
            c.itemconfigure(
                self._chrome["spike"], fill=fill, outline=theme.SPIKE_ARMED
            )
            c.itemconfigure(
            ...
            c.itemconfigure(
                self._chrome["spike_text"], text="defused", fill=theme.SPIKE_SAFE
            )
        elif snap.spike_state == SPIKE_EXPLODED:
            c.itemconfigure(
                self._chrome["spike"], fill=theme.SPIKE_BOOM,
                outline=theme.SPIKE_BOOM,
            )
            c.itemconfigure(
                self._chrome["spike_text"], text="exploded", fill=theme.SPIKE_BOOM
            )
