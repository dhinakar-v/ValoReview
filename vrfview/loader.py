...
from vrf_reader import VrfFile, VrfError
...
def _from_document(
    ...
    for ev in sorted(events, key=lambda e: e["time2_ms"]):
        ...
        t = int(ev["time2_ms"])
        args = ev.get("args") or []
        if group == "characterDeath" and len(args) >= 3:
            replay.kills.append(Kill(t_ms=t, killer=int(args[1]), victim=int(args[2])))
        elif group == "characterUltimateUsed" and len(args) >= 2:
    ...
    if not replay.rounds:
        raise VrfError(f"{source}: no roundStarted events, nothing to play back")
    return replay
...
def _rounds(events: list, length_ms: int) -> list:
    ...
    for i, (start, index) in enumerate(starts):
        ...
        rounds.append(
            Round(number=index + 1, index=index, start_ms=start, end_ms=end)
        )
    return rounds
