...
class Callbacks:
    ...
    toggle_layer: Callable[[str, bool], None]
...
class TimelineStrip:
    ...
    def __init__(self, master: tk.Misc, replay: Replay, cb: Callbacks) -> None:
        ...
        self.canvas = tk.Canvas(
            master, bg=theme.PANEL, height=STRIP_HEIGHT, highlightthickness=0, bd=0
        )
        self._width = 0
        self._playhead = self.canvas.create_line(
            0, 0, 0, 0, fill=theme.PLAYHEAD, width=2
        )
        self._ghost = self.canvas.create_line(
            0, 0, 0, 0, fill=theme.FAINT, width=1, state="hidden"
        )
    ...
    def x_to_ms(self, x: float) -> int:
        ...
        return int(round(frac * self.replay.length_ms))
    ...
    def _rebuild(self) -> None:
        ...
        for i, rnd in enumerate(self.replay.rounds):
            ...
            c.create_rectangle(
                x0, BAND_TOP, x1, BAND_BOTTOM, fill=shade, outline="", tags="static"
            )
            width = x1 - x0
            if width >= 40:
                text = f"R{rnd.number}"
            ...
            if text:
                c.create_text(
                    (x0 + x1) / 2, (BAND_TOP + BAND_BOTTOM) / 2, text=text,
                    fill=theme.MUTED, font=("Segoe UI", 8), tags="static",
                )
            self._chips[rnd.number] = c.create_text(
                (x0 + x1) / 2, CHIP_ROW, text="", fill=theme.MUTED,
                font=("Segoe UI Semibold", 9), tags="static",
            )
        ...
        for kill in self.replay.kills:
            ...
            c.create_line(
                x, TICK_TOP, x, TICK_BOTTOM - 6, fill=colour, width=1, tags="static"
            )
        for ult in self.replay.ultimates:
            x = self.ms_to_x(ult.t_ms)
            c.create_line(
                x, TICK_TOP, x, TICK_BOTTOM, fill=theme.ULT, width=1, tags="static"
            )
        for spike in self.replay.spike:
            glyph, colour = _SPIKE_GLYPH.get(spike.kind, ("?", theme.MUTED))
            c.create_text(
                self.ms_to_x(spike.t_ms), TICK_BOTTOM - 3, text=glyph, fill=colour,
                font=("Segoe UI", 7), tags="static",
            )
        if self.replay.side_swap_ms is not None:
            x = self.ms_to_x(self.replay.side_swap_ms)
            c.create_line(
                x, 2, x, STRIP_HEIGHT - 2, fill=theme.ACCENT, width=1, dash=(3, 2),
                tags="static",
            )
            c.create_text(
                x + 4, 6, text="swap", anchor="w", fill=theme.ACCENT,
                font=("Segoe UI", 7), tags="static",
            )
        ...
        for rnd in self.replay.rounds:
            ...
            if rnd.winner == TEAM_A:
                self.canvas.itemconfigure(
                    item, text="A", fill=theme.team_colour(TEAM_A)
                )
            elif rnd.winner == TEAM_B:
                self.canvas.itemconfigure(
                    item, text="B", fill=theme.team_colour(TEAM_B)
                )
            else:
...
class ControlBar:
    ...
    def __init__(self, master: tk.Misc, replay: Replay, cb: Callbacks) -> None:
        ...
        ttk.Button(left, text="|<", width=3,
                   command=lambda: cb.step_round(-1)).pack(side="left", padx=1)
        ttk.Button(left, text="<", width=3,
                   command=lambda: cb.step_event(-1)).pack(side="left", padx=1)
        self.play_button = ttk.Button(left, text="Play", width=6,
        ...
        self.time_label = ttk.Label(
            self.frame, text="", font=("Consolas", 10), style="Read.TLabel"
        )
        ...
        ttk.Button(right, text="Provenance",
                   command=cb.show_provenance).pack(side="right", padx=4)
        for name, label in (
            ("kills", "Kills"), ("trails", "Trails"), ("ults", "Ults"),
            ("spike", "Spike"), ("kd", "K/D"),
        ):
            var = tk.BooleanVar(value=name != "trails")
            self._layer_vars[name] = var
            ttk.Checkbutton(
                right, text=label, variable=var,
                command=lambda n=name, v=var: cb.toggle_layer(n, v.get()),
