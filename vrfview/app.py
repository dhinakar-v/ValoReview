...
Four rows on a grid: the scene takes every spare pixel, and the strip, the
control bar and the status line are fixed height and docked below it, so
resizing the window grows the field and never the furniture.
...
class ViewerApp(tk.Tk):
    ...
    def __init__(self, replay: Replay) -> None:
        ...
        self.bar = ControlBar(self, replay, cb)
        ...
        self.scene.widget.grid(row=0, column=0, sticky="nsew")
        self.strip.widget.grid(row=1, column=0, sticky="ew")
        self.bar.widget.grid(row=2, column=0, sticky="ew")
    ...
    def _style(self) -> None:
        ...
        style.configure(".", background=theme.PANEL, foreground=theme.TEXT,
                        borderwidth=0, focuscolor=theme.PANEL)
        ...
        style.configure("Status.TLabel", background=theme.BACKGROUND,
                        foreground=theme.MUTED, padding=(10, 4),
                        font=("Segoe UI", 8))
        style.configure("TButton", background=theme.PANEL_EDGE,
                        foreground=theme.TEXT, padding=(6, 3))
        style.map("TButton",
    ...
    def _redraw(self) -> None:
        ...
        self.bar.refresh(snap, self.clock.playing, self.clock.speed)
    ...
    def _status_text(self, snap) -> str:
        ...
        return (
            ...
            f"{a}-{b}   |   Not in file: player names, agents, ATK/DEF sides, "
            f"positions"
        )
    ...
    def _step_round(self, direction: int) -> None:
        ...
        if direction > 0:
            ...
            if current is not None and t - current.start_ms > 1500:
    ...
    def _on_hover_node(self, actor_id: int | None) -> None:
        if actor_id is None:
            ...
            merged = (
                ...
                if player and player.merged_from else ""
            )
    ...
    def _show_provenance(self) -> None:
        ...
        text = tk.Text(
            top, bg=theme.BACKGROUND, fg=theme.TEXT, bd=0, wrap="word",
            font=("Consolas", 9), padx=14, pady=12,
            insertbackground=theme.TEXT,
        )
        text.pack(fill="both", expand=True)
        text.insert("end", _provenance_text(self.replay))
...
def _provenance_text(replay: Replay) -> str:
    lines = [
        "READ FROM THE FILE",
        ...
        f"  map name          {replay.map_name}, from the public codename list",
    ]
    lines += [f"  {note}" for note in replay.notes]
    lines += [
        "",
        "NOT IN THE FILE",
        "  player names / Riot IDs   absent",
        "  agent per actor id        8 codenames are visible in block000 but",
        "                            cannot be linked to the event actor ids",
...
def run(replay: Replay) -> int:
    """Open the window and block until it closes."""
    app = ViewerApp(replay)
    app.mainloop()
    return 0
