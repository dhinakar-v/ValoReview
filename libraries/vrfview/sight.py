"""
An approximate sight cone, raycast against the radar image's own silhouette.

This project has no collision data.  There is no navmesh, no wall geometry and
no height information anywhere in `assets/` -- a map entry is a radar PNG, four
transform scalars and a list of point callouts, and that is the entire spatial
model.  A true line of sight is therefore not implementable here, and this
module does not claim to be one.

What it does have is a measurement.  Every `minimap.png` Riot publishes is
RGBA, and the area outside the playable space is *fully transparent*: 57% of
Abyss, 67% of Ascent, 72% of Bind, 60% of Breeze.  That silhouette is real data
about the map, published by Riot, and marching a ray until it leaves it gives
an occluder that is measured rather than invented.
...
class SightMap:
    """One map's playable silhouette, as a square bitmask of open cells."""
    ...
    def from_image(cls, image, size: int = GRID) -> SightMap:
        """
        Build from a PIL image, reading only its alpha channel.
        ...
        at the player's feet reads as a bug.
        """
...
def _march(
    ...
    for i in range(1, steps + 1):
        ...
        u = u0 + du * travelled
        v = v0 + dv * travelled
        if i > SEED_CELLS and sight.blocked(u, v):
