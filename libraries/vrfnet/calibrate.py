...
CALIBRATION_FILE = Path(__file__).resolve().with_name("calibrated.json")

# What "the layout is settled" means: nearly every packet consumed cleanly, and
# a clear gap to the next best layout rather than a coin flip between two.
_DECISIVE_RATE = 0.99
...
class Calibration:
    ...
    def is_decisive(self) -> bool:
        ...
        return self.rate >= _DECISIVE_RATE and self.margin >= _DECISIVE_MARGIN
...
def save(calibration: Calibration, path: str | Path = CALIBRATION_FILE) -> None:
    payload = {
...
def load(path: str | Path = CALIBRATION_FILE) -> Features:
