...
class BitReader:
    ...
    def __init__(self, buf: bytes, num_bits: int | None = None):
        ...
        if self.num_bits > len(buf) * 8:
            msg = f"num_bits={self.num_bits} exceeds buffer of {len(buf)} bytes"
            raise NetError(msg)
        self.pos = 0
    ...
    def _need(self, n: int) -> None:
        if n < 0:
            msg = f"negative read of {n} bits at bit {self.pos}"
            raise NetError(msg)
        if self.pos + n > self.num_bits:
            msg = f"read past end: want {n} bits at bit {self.pos} of {self.num_bits}"
            raise NetError(msg)
    ...
    def __repr__(self) -> str:
        return f"<BitReader pos={self.pos}/{self.num_bits} ({len(self.buf)} bytes)>"
