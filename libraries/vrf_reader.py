...
# An event payload opens with a uint32 type id ahead of the group marker.
EVENT_TYPE_ID_SIZE = 4

# FString scan bounds.  A positive length is ANSI and needs at least one
# character plus the trailing NUL; a negative length is UTF-16, and anything
# longer than 4096 characters is noise rather than a string.
_MIN_ANSI_FSTRING_LEN = 2
_SHORTEST_UTF16_FSTRING_LEN = -2
_LONGEST_UTF16_FSTRING_LEN = -4096

# Printable ASCII, used to tell a real string apart from a byte run.
_ASCII_SPACE = 32
_ASCII_DEL = 127

# How many NetGUID table entries the --decode summary prints before eliding.
_NAMES_SHOWN = 15
...
class Reader:
    ...
    def _unpack(self, fmt: str, size: int):
        if self.pos + size > len(self.buf):
            msg = f"read past end at {self.pos}"
            raise VrfError(msg)
...
class ContainerHeader:
    ...
    def recorded_utc(self):
        ...
        return _dt.datetime.fromtimestamp(secs, _dt.UTC)
...
class DataBlock:
    ...
    def oodle_codec(self, data: bytes) -> str:
        ...
        if b0 & 0x0F != _OODLE_HEADER_NIBBLE:
...
# Low nibble of an Oodle block header's first byte; anything else is not Oodle.
_OODLE_HEADER_NIBBLE = 0x0C
...
class Oodle:
    ...
    def __init__(self, dll_path: str | Path):
        ...
        fn.argtypes = [
            ctypes.c_char_p,
            ctypes.c_ssize_t,  # compressed buffer + size
            ctypes.c_char_p,
            ctypes.c_ssize_t,  # output buffer + size
            ctypes.c_int,
            ctypes.c_int,
            ctypes.c_int,  # fuzzSafe, checkCRC, verbosity
            ctypes.c_void_p,
            ctypes.c_ssize_t,  # decBufBase, decBufSize
            ctypes.c_void_p,
            ctypes.c_void_p,  # callback, callback userdata
            ctypes.c_void_p,
            ctypes.c_ssize_t,  # scratch, scratch size
    ...
    def decompress(self, blob: bytes, out_size: int) -> bytes:
        ...
        if n != out_size:
            msg = f"OodleLZ_Decompress returned {n}, expected {out_size}"
            raise VrfError(msg)
        return out.raw[:n]
...
class VrfFile:
    ...
    def _parse_container_header(self) -> ContainerHeader:
        ...
        if magic != CONTAINER_MAGIC:
            msg = f"not a .vrf: magic 0x{magic:08X}, expected 0x{CONTAINER_MAGIC:08X}"
            raise VrfError(msg)
        file_version = r.u32()
        unknown_08 = r.u32()
        guid = r.guid()
    ...
    def _walk_chunks(self) -> Iterator[Chunk]:
        ...
        while off + 8 <= end:
            ...
            if off + 8 + size > end:
                msg = f"truncated chunk at 0x{off:X}: type={ctype} size={size}"
                raise VrfError(msg)
            yield Chunk(ctype, off + 8, size)
            off += 8 + size
        if off != end:
            msg = f"trailing {end - off} bytes after last chunk"
            raise VrfError(msg)
    ...
    def _parse_demo_header(self) -> DemoHeader:
        ...
        magic = r.u32()
        if magic != DEMO_MAGIC:
            msg = f"bad demo header magic 0x{magic:08X}"
            raise VrfError(msg)
...
def _iter_ansi_strings(buf: bytes) -> Iterator[str]:
    ...
    while i + 4 <= end:
        ...
        if _MIN_ANSI_FSTRING_LEN <= ln <= end - i - 4:
            stop = i + 4 + ln
            text = buf[i + 4 : stop]
...
def _iter_fstrings(buf: bytes, start: int = 0) -> Iterator[str]:
    ...
    while i + 4 <= end:
        ...
        if _MIN_ANSI_FSTRING_LEN <= n <= end - i - 4:  # ANSI
            stop = i + 4 + n
            text = buf[i + 4 : stop]
            if text.endswith(b"\x00") and _is_text(text[:-1]):
                yield text[:-1].decode("utf-8", "replace")
                i = stop
                continue
        elif _LONGEST_UTF16_FSTRING_LEN <= n <= _SHORTEST_UTF16_FSTRING_LEN:  # UTF-16
            width = -n * 2
            stop = i + 4 + width
            if stop <= end:
                text = buf[i + 4 : stop]
                if (
                    text.endswith(b"\x00\x00")
                    and _is_text(text[:-2:2])
                    and all(c == 0 for c in text[1::2])
                ):
...
def _is_text(b: bytes) -> bool:
    return bool(b) and all(
        c in (9, 10, 13) or _ASCII_SPACE <= c < _ASCII_DEL for c in b
    )
...
def cmd_decode(vrf: VrfFile, index: int, dll: str | None, outfile: str | None) -> None:
    ...
    if not 0 <= index < len(blocks):
        msg = f"block {index} out of range (0..{len(blocks) - 1})"
        raise VrfError(msg)
    b = blocks[index]
    ...
    print(
        f"NetGUID table   {len(names)} entries "
        f"({len(paths)} asset paths, {len(names) - len(paths)} property names)",
    )
    for n in names[:_NAMES_SHOWN]:
        print(f"  {n}")
    if len(names) > _NAMES_SHOWN:
        print(f"  ... {len(names) - _NAMES_SHOWN} more")
...
def cmd_dump_json(vrf: VrfFile, outdir: str | Path) -> None:
    out = Path(outdir)
    out.mkdir(parents=True, exist_ok=True)
    for i, raw in enumerate(vrf.demo.raw_json):
        dest = out / f"{vrf.path.stem}.meta{i}.json"
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--players",
        action="store_true",
        help="print players from metadata",
    )
    ...
    ap.add_argument(
        "--blocks",
        action="store_true",
        help="print the compressed block table",
    )
    ap.add_argument(
        "--decode",
        type=int,
