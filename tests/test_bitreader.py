...
from vrfnet.bitreader import BitReader, NetError  # noqa: E402
...
class TestBitOrder(unittest.TestCase):
    ...
    def test_read_bits_first_bit_is_least_significant(self):
        ...
        self.assertEqual(r.read_bits(4), 0b0110)
        self.assertEqual(r.read_bits(4), 0b1101)

    def test_spans_byte_boundary(self):
        r = BitReader(bytes([0xFF, 0x00]))
        self.assertEqual(r.read_bits(4), 0xF)
        self.assertEqual(r.read_bits(8), 0x0F)
        self.assertEqual(r.read_bits(4), 0x0)

    def test_peek_does_not_advance(self):
        r = BitReader(bytes([0xAB, 0xCD]))
        first = r.peek_bits(12)
        self.assertEqual(r.peek_bits(12), first)
        self.assertEqual(r.pos, 0)
        self.assertEqual(r.read_bits(12), first)
...
class TestIntPacked(unittest.TestCase):
    ...
    def test_hand_computed(self):
        for value, encoded in self.HAND:
            with self.subTest(value=value):
                self.assertEqual(BitReader(encoded).read_int_packed(), value)
    ...
    def test_hand_computed_consumes_exact_bits(self):
        for value, encoded in self.HAND:
            with self.subTest(value=value):
                r = BitReader(encoded)
                r.read_int_packed()
                self.assertEqual(r.pos, len(encoded) * 8)
                self.assertTrue(r.at_end())
        ...
        for offset in range(8):
            for value in values:
                with self.subTest(offset=offset, value=value):
                    ...
                    r.skip_bits(offset)
                    self.assertEqual(r.read_int_packed(), value)
                    self.assertTrue(r.at_end())
    ...
    def test_sequence_of_packed_ints_stays_in_sync(self):
        ...
        r = w.reader()
        r.read_bit()
        self.assertEqual([r.read_int_packed() for _ in values], values)
        self.assertTrue(r.at_end())
            ...
            for value in range(min(max_value, 12)):
                for offset in (0, 1, 5, 7):
                    with self.subTest(max=max_value, value=value, offset=offset):
                        ...
                        r.skip_bits(offset)
                        self.assertEqual(r.read_int(max_value), value)
                        self.assertTrue(r.at_end())
...
class TestScalars(unittest.TestCase):
    ...
    def test_read_bytes_unaligned(self):
        ...
        r = w.reader()
        r.skip_bits(5)
        self.assertEqual(r.read_bytes(4), payload)

    def test_read_bits_bytes_pads_to_whole_bytes(self):
        r = BitReader(bytes([0b101, 0x00]))
        self.assertEqual(r.read_bits_bytes(3), bytes([0b101]))
