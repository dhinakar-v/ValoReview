...
def dump_demo_header(vrf: VrfFile) -> dict:
    ...
    return {
        ...
        "game_specific_data": [
            ...
            for i, (raw, parsed) in enumerate(zip(d.raw_json, d.json_blobs))
        ],
    }
...
def decode_block(vrf: VrfFile, index: int, b, oodle, opts) -> dict:
    ...
    if opts.payload_dir:
        os.makedirs(opts.payload_dir, exist_ok=True)
        suffix = f"_{b.label}" if b.label else ""
        dest = os.path.join(
            opts.payload_dir, f"block{index:03d}_{b.chunk.name.lower()}{suffix}.bin"
        )
...
def build(vrf: VrfFile, oodle, opts) -> dict:
    ...
    doc = {
        "source": {
            "path": os.path.abspath(vrf.path),
            "file_name": os.path.basename(vrf.path),
            "size_bytes": len(vrf.data),
            "sha256": _sha256(vrf.data),
        },
        "oodle": {"dll": oodle.path if oodle else None, "used": oodle is not None},
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--payload-dir", metavar="DIR", help="write each decompressed block as .bin"
    )
    ap.add_argument(
        "--inline-payloads",
        action="store_true",
